import { pool } from "../config/database.js";

// CREATE Trip
const createTrip = async (req, res) => {
  try {
    const {
      title,
      description,
      img_url,
      num_days,
      start_date,
      end_date,
      total_cost,
    } = req.body;
    const results = await pool.query(
      `INSERT INTO trips (title, description, img_url, num_days, start_date, end_date, total_cost)
             VALUES($1, $2, $3, $4, $5, $6, $7)
             RETURNING *`,
      [title, description, img_url, num_days, start_date, end_date, total_cost]
    );
    res.status(201).json(results.rows[0]);
  } catch (error) {
    res.status(409).json({ error: error.message });
  }
};

// GET All Trips
const getTrips = async (req, res) => {
  try {
    const results = await pool.query("SELECT * FROM trips ORDER BY id ASC");
    res.status(200).json(results.rows);
  } catch (error) {
    res.status(409).json({ error: error.message });
  }
};

// GET Trip by ID
const getTrip = async (req, res) => {
  try {
    const id = parseInt(req.params.id);
    const results = await pool.query("SELECT * FROM trips WHERE id = $1", [id]);
    res.status(200).json(results.rows[0]);
  } catch (error) {
    res.status(409).json({ error: error.message });
  }
};

// UPDATE Trip
const updateTrip = async (req, res) => {
  try {
    const id = parseInt(req.params.id);
    const {
      title,
      description,
      img_url,
      num_days,
      start_date,
      end_date,
      total_cost,
    } = req.body;
    const results = await pool.query(
      `UPDATE trips
             SET title = $1, description = $2, img_url = $3, num_days = $4, start_date = $5, end_date = $6, total_cost = $7
             WHERE id = $8 RETURNING *`,
      [
        title,
        description,
        img_url,
        num_days,
        start_date,
        end_date,
        total_cost,
        id,
      ]
    );
    res.status(200).json(results.rows[0]);
  } catch (error) {
    res.status(409).json({ error: error.message });
  }
};

// DELETE Trip
const deleteTrip = async (req, res) => {
  try {
    const id = parseInt(req.params.id);
    // Before deleting a trip, we must delete related activities due to the foreign key constraint
    await pool.query("DELETE FROM activities WHERE trip_id = $1", [id]);
    // Also delete from join tables (optional for this lab but good practice)
    await pool.query("DELETE FROM trips_destinations WHERE trip_id = $1", [id]);
    await pool.query("DELETE FROM trips_users WHERE trip_id = $1", [id]);

    // Now delete the trip itself
    const results = await pool.query(
      "DELETE FROM trips WHERE id = $1 RETURNING *",
      [id]
    );
    if (results.rowCount === 0) {
      return res.status(404).json({ message: "Trip not found" });
    }
    // Return the deleted trip data (optional, could just send 204 No Content)
    res.status(200).json(results.rows[0]);
  } catch (error) {
    res.status(409).json({ error: error.message });
  }
};

export default {
  createTrip,
  getTrips,
  getTrip,
  updateTrip,
  deleteTrip,
};
